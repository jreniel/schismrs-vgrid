...
impl VQS {
    ...
    pub fn try_from_file(hgrid: &Hgrid, vgrid_path: &Path) -> Result<Self, VQSLoadError> {
        ...
        if node_count != hgrid.nodes().len() {
            return Err(VQSLoadError::NodeCountMismatch(hgrid.nodes().len(), node_count));
        }
        
        // Convert bottom indices to kbp (actual number of levels per node)
        let kbp: Vec<usize> = bottom_indices.iter()
            .map(|&idx| nvrt + 1 - idx)
            .collect();
        ...
        for level in 1..=nvrt {
            ...
            if parts.is_empty() {
                return Err(VQSLoadError::InvalidFormat(
                    format!("Empty line at level {}", level)
                ));
            }
            
            // First value is the level number
            let level_num: usize = parts[0].parse()
                .map_err(|_| VQSLoadError::ParseError(
                    format!("Failed to parse level number at level {}", level)
                ))?;
            
            if level_num != level {
                return Err(VQSLoadError::InvalidFormat(
                    format!("Expected level {}, got {}", level, level_num)
                ));
            }
            ...
            if parts.len() - 1 != node_count {
                return Err(VQSLoadError::InconsistentDimensions(
                    node_count, parts.len() - 1
                ));
            }
            
            for (node_idx, sigma_str) in parts[1..].iter().enumerate() {
                let sigma_val: f64 = sigma_str.parse()
                    .map_err(|_| VQSLoadError::ParseError(
                        format!("Failed to parse sigma at level {}, node {}", level, node_idx)
                    ))?;
        ...
        let (extracted_depths, extracted_levels) = Self::quick_extract_master_grids(&sigma_vqs, &kbp, hgrid)
            .map_err(|e| VQSLoadError::ParseError(format!("Failed to extract master grids: {}", e)))?;
        ...
        let transform = stretching.transform(hgrid, &extracted_depths, &extracted_levels)
    ...
    pub fn extract_master_grids(&self, hgrid: &Hgrid) -> Result<(Vec<f64>, Vec<usize>), ReconstructionError> {
        ...
        for (idx, &depth) in depths.iter().enumerate() {
            if depth < 0.0 {  // Negative depth means underwater
        ...
        let success_rate = self.validate_master_grids(
            &master_depths, 
            &master_levels, 
            &depth_level_pairs,
            hgrid
    ...
    fn debug_sigma_structures(&self, wet_nodes: &[usize], depths: &[f64]) {
        ...
        for target_depth in samples.iter() {
            ...
            if let Some(node_idx) = best_node {
                ...
                println!("\nNode {} (depth={:.2}m, levels={}):", node_idx, actual_depth, levels);
    ...
    fn analyze_linear_relationship(&self, depth_level_pairs: &[(f64, usize)]) {
        ...
        let sum_xy: f64 = depth_level_pairs.iter().map(|(d, l)| d.ln() * (*l as f64)).sum();
        ...
        // Calculate R-squared
        let y_mean = sum_y / n;
        let ss_tot: f64 = depth_level_pairs.iter()
            .map(|(_, l)| {
                let y = *l as f64;
                (y - y_mean) * (y - y_mean)
            })
        ...
        let ss_res: f64 = depth_level_pairs.iter()
            .map(|(d, l)| {
        ...
        println!("\n=== Linear Relationship Analysis ===");
        println!("Log-linear fit: levels = {:.3} * ln(depth) + {:.3}", slope, intercept);
        println!("R-squared: {:.3} ({:.1}% of variance explained)", r_squared, r_squared * 100.0);
    ...
    fn identify_master_grids(&self, depth_level_pairs: &[(f64, usize)]) -> Result<(Vec<f64>, Vec<usize>), ReconstructionError> {
        info!("Identifying master grids from {} depth-level pairs", depth_level_pairs.len());
    ...
    fn validate_master_grids(
        ...
        println!("\nValidation results:");
        println!("  Matching nodes: {} / {}", matching_count, depth_level_pairs.len());
    ...
    fn interpolate_levels(&self, depth: f64, master_depths: &[f64], master_levels: &[usize]) -> usize {
    ...
    fn write_non_matching_csv(&self, non_matching: &[(usize, f64, usize, usize)]) -> Result<(), ReconstructionError> {
        let mut wtr = csv::Writer::from_path("non_matching_nodes.csv")?;
        wtr.write_record(&["node_index", "depth_m", "actual_levels", "expected_levels", "difference"])?;
